$(function(){
	...
	complexNearby.keyup(function(){
		...
		requestOld = val;
		if(!requestInProgress && val.length == 4 && requestOldSearch != val){
			...
			$.post("/index.php", { jsRequest: '1', cmd: 'complexNearby', 'zipCode' : val }, function( data ){
				if(data.status){
					...
					if(failed){
						complexNearbyUl.append('<li class="result noListStyle">Vi kan ikke finne noen senter i nærheten av deg</li>');
					}
				}
				requestInProgress = false;
			}, "json");
				...
				if(data.status){
					$.each(data.result, function(i, item){
						...
						if( isComplexPt > 0 )
						{
							...
							ptListHtml = "<ul>" + ptListInnerHtml + "</ul>";
							isComplexPt = true;
						}
	...
	if(!getCookie('allowCookie')){
		...
		//$('body').prepend('<div style="width:100%; padding:10px 0; background-color:#007fc6; text-align:center; color:#fff; font-size:14px;">Denne websiden benytter cookies. Ved fortsatt bruk av websiden godkjenner du benyttelsen av cookies. For ytterligere informasjon se <a target="_blank" href="http://no.wikipedia.org/wiki/Informasjonskapsel" style="color:#D1D1E9;">her</a>.</div>');
